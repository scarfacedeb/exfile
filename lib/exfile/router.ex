defmodule Exfile.Router do
  ...
  defp stream_file(%{assigns: %{exfile_local_file: %LocalFile{path: path}}} = conn)
       when not is_nil(path) do
    io = File.open!(path)

    conn
    |> assign(:exfile_local_file, %LocalFile{io: io})
    |> stream_file()
  end

  defp stream_file(%{assigns: %{exfile_local_file: %LocalFile{io: io}}} = conn)
       when not is_nil(io) do
  ...
  defp set_content_type(conn, filename) do
    mime_type =
      case filename do
        nil -> "application/octet-stream"
        _ -> MIME.from_path(filename)
      end
